...
def test_settings_from_env(monkeypatch: pytest.MonkeyPatch):
    ...
    monkeypatch.setenv("PM_RPC_WS_URLS", "wss://custom.rpc.com")
    ...
    assert s.rpc_ws_urls == "wss://custom.rpc.com"
...
def test_settings_backward_compat_alchemy_env(monkeypatch: pytest.MonkeyPatch):
    ...
    monkeypatch.delenv("PM_RPC_WS_URLS", raising=False)
    ...
    assert s.rpc_ws_urls == "wss://legacy.alchemy.com"


def test_settings_backward_compat_single_url(monkeypatch: pytest.MonkeyPatch):
    """Legacy PM_RPC_WS_URL (singular) should still work via AliasChoices."""
    monkeypatch.delenv("PM_RPC_WS_URLS", raising=False)
    monkeypatch.setenv("PM_RPC_WS_URL", "wss://single.rpc.com")

    from polymarket_pipeline.live.settings import Settings

    s = Settings(_env_file=None)
    assert s.rpc_ws_urls == "wss://single.rpc.com"


def test_dashboard_settings_defaults(monkeypatch: pytest.MonkeyPatch):
    """Dashboard fields should have sensible defaults."""
    monkeypatch.setenv("PM_RPC_WS_URLS", "wss://test.example.com")
...
def test_settings_rpc_urls_has_free_defaults(monkeypatch: pytest.MonkeyPatch):
    """rpc_ws_urls defaults to multiple free public Polygon RPC endpoints."""
    monkeypatch.delenv("PM_RPC_WS_URLS", raising=False)
    monkeypatch.delenv("PM_RPC_WS_URL", raising=False)
    ...
    s = Settings(_env_file=None)
    urls = [u.strip() for u in s.rpc_ws_urls.split(",")]
    assert len(urls) >= 3
    assert "publicnode" in s.rpc_ws_urls
