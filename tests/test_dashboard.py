...
@pytest.fixture
async def mock_checker(monkeypatch: pytest.MonkeyPatch) -> QualityChecker:
...
async def test_build_html_contains_pipeline_state(mock_checker: QualityChecker):
    from polymarket_pipeline.live.dashboard import build_dashboard_html

    html = await build_dashboard_html(mock_checker, refresh_s=5)
    assert '<meta http-equiv="refresh"' in html
    assert "DEGRADED" in html or "READY" in html or "CHECKING" in html


async def test_build_html_contains_producer_table(mock_checker: QualityChecker):
    from polymarket_pipeline.live.dashboard import build_dashboard_html
...
async def test_build_html_contains_check_results(mock_checker: QualityChecker):
    from polymarket_pipeline.live.dashboard import build_dashboard_html
...
async def test_build_html_contains_gap_section(mock_checker: QualityChecker):
    from polymarket_pipeline.live.dashboard import build_dashboard_html
...
async def test_make_asgi_app_callable(mock_checker: QualityChecker):
