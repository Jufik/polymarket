...
class ManagedSlot:
    ...
    async def _recv_loop(
        ...
        reconnect_fut: asyncio.Future[Any] = asyncio.ensure_future(self._reconnect_event.wait())
        rotate_fut: asyncio.Future[Any] = asyncio.ensure_future(self._rotate_event.wait())
        update_fut: asyncio.Future[Any] = asyncio.ensure_future(self._update_event.wait())
        ...
        try:
            while True:
                ...
                if update_fut in done:
                    ...
                    update_fut = asyncio.ensure_future(self._update_event.wait())
                    continue
    ...
    async def _hot_swap(self, old_ws: Any) -> Any | None:
        ...
        Returns the new WS on success, ``None`` on failure.
        """
        try:
