...
import structlog
import websockets
from pm_core.constants import NEGRISK_UMA_ADAPTER, UMA_CTF_ADAPTER_V3
from pm_core.token_map import TokenMap

from pm_ingest.base import BaseIngestor
from pm_ingest.dedup import TradeDedup
from pm_ingest.normalize.decode import ORDER_FILLED_SIG, decode_rpc_log, decode_settled_price
    ...
    async def _handle_resolution_message(self, raw: str) -> None:
        ...
        payload = {
            ...
            "payload": {
                ...
                "block_number": (
                    int(result["blockNumber"], 16) if result.get("blockNumber") else None
                ),
            },
            "timestamp": time.time(),
        }
