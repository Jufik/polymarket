...
Usage (fast path -- preferred):
    from pm_backtest.harness import run_fast_backtest, print_summary
    from research.strategies.example import ExampleStrategy
    from pm_strategy.config import StrategyConfig
    from pm_strategy.types import ExecutionMode
...
Usage (legacy):
    import asyncio
    from pm_backtest.harness import load_compact_trades, run_backtest, print_summary
...
from pm_strategy.execution.calibrate import (
    calibrate_spreads,
    calibrate_volumes,
)
from pm_strategy.execution.gateway import ExecutionGateway
from pm_strategy.execution.realistic import (
    FillModelConfig,
    RealisticFillSimulator,
)
from pm_strategy.execution.simulated import SimulatedExecutor
from pm_strategy.ledger.analytics import (
    LedgerSummary,
    compute_summary,
)
from pm_strategy.ledger.parquet import ParquetLedger
...
if TYPE_CHECKING:
    ...
    from pm_strategy.config import StrategyConfig
    from pm_strategy.protocols import Strategy
...
def run_fast_backtest(
    ...
    *,
    ...
    """
    from pm_backtest.replay import load_replay_resolutions, load_replay_trades
    from pm_backtest.sync_runner import SyncReplayRunner, _run_coro
...
def print_summary(summary: LedgerSummary, name: str = "") -> None:
    ...
    print(f"  Wins/Losses:     {summary.win_count}/{summary.loss_count} (pending: {pending})")
