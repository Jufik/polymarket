...
sorted Parquet files. Converts to lightweight ReplayTick objects (~0.5 us each
vs ~10 us for NormalizedTrade).

Usage:
    from pm_backtest.replay import load_replay_trades, load_replay_resolutions
...
class ReplayTick:
    ...
    Construction: ~0.5 us vs ~10 us for Pydantic NormalizedTrade.
...
def load_replay_trades(
    *,
    ...
    # Convert to ReplayTick objects (~0.5 us each)
    t1 = time.time()
...
def load_replay_resolutions(
    ...
    """
    from pm_backtest.runners.replay import (
        MarketResolution,
    )
