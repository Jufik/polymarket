...
from pm_strategy.protocols import Executor
from pm_strategy.types import FillStatus

logger = structlog.get_logger(__name__)


# Inline the pipeline state types to avoid depending on polymarket_pipeline.quality.state
...
class ExecutionGateway:
    ...
    async def submit(self, intent: TradeIntent) -> Fill:
        ...
        """
        from pm_strategy.types import Fill as FillType
        ...
        if self._quality_state is not None:
            state = getattr(self._quality_state, "current", None)
            if state is not None and state not in (PipelineState.CHECKING, PipelineState.READY):
                logger.warning(
                    "gateway_rejected",
            ...
            if fill.status in (FillStatus.FILLED, FillStatus.PARTIAL):
                ...
                if reserved != actual:
                    self._strategy_spent[intent.strategy] = (
                        self._strategy_spent.get(intent.strategy, 0.0) - reserved + actual
                    )
            else:
