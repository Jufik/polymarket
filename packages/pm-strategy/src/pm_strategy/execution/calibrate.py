...
def calibrate_spreads(
    ...
    *,
    ...
    df = df.with_columns(
        pl.col("price").diff().over("condition_id").alias("dp"),
    )
...
def calibrate_volumes(
    ...
    result = df.group_by("condition_id").agg(
        pl.col("amount_usd").cast(pl.Float64).sum().alias("volume")
    )
...
def _median_abs_change(df: pl.DataFrame, min_trades: int) -> dict[str, float]:
...
def _roll_estimator(df: pl.DataFrame, min_trades: int) -> dict[str, float]:
    ...
    df = df.with_columns(
        pl.col("dp").shift(1).over("condition_id").alias("dp_lag"),
    )
    ...
    for group_name, group_df in df.group_by("condition_id"):
        ...
        cov = sum((a - mean_dp) * (b - mean_lag) for a, b in zip(dp, dp_lag, strict=True)) / (n - 1)
