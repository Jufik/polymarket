...
class LedgerBackend(Protocol):
    ...
    async def enrich_resolutions(self, resolutions: dict[str, tuple[str, float]]) -> int:
...
def compute_pnl(
    ...
    if record.side == "BUY":
        pnl_gross = (1.0 - record.fill_price) * qty_tokens if won else -record.fill_size_usd
    else:  # SELL
        pnl_gross = (record.fill_price - 1.0) * qty_tokens if won else record.fill_size_usd
