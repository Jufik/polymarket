...
from pm_strategy.execution.monitor import PositionMonitor
from pm_strategy.helpers import apply_fill_to_position, check_risk_gate
from pm_strategy.types import FillStatus, MarketInfo
...
if TYPE_CHECKING:
    ...
    from pm_strategy.protocols import (
        FeatureBackend,
        FeatureProvider,
        Strategy,
    )
    from pm_strategy.types import Fill, TradeIntent
...
class LiveRunner:
    ...
    async def _build_intent_metadata(
        ...
        if hasattr(strategy, "get_rationale"):
            try:
                metadata["rationale"] = strategy.get_rationale(intent.condition_id, trade, self.ctx)
            except Exception:
    ...
    async def _resolve_sell_size(self, intent: TradeIntent) -> TradeIntent | None:
        ...
        from pm_strategy.types import TradeIntent as TI
        ...
        if qty <= 0:
            ...
            return None
        avg_entry = pos.avg_entry_yes if intent.outcome == "YES" else pos.avg_entry_no
        ...
        for strategy, config in self.strategies:
            ...
            if intents:
                for intent in intents:
                    ...
                    metadata = await self._build_intent_metadata(intent, trade, strategy)
    ...
    def handle_orderbook(self, data: dict[str, Any]) -> None:
        ...
        bids = tuple((float(lvl[0]), float(lvl[1])) for lvl in raw_bids) if raw_bids else ()
        asks = tuple((float(lvl[0]), float(lvl[1])) for lvl in raw_asks) if raw_asks else ()
        bid_depth = (
            float(data["bid_depth_usd"]) if "bid_depth_usd" in data else sum(p * s for p, s in bids)
        )
        ask_depth = (
            float(data["ask_depth_usd"]) if "ask_depth_usd" in data else sum(p * s for p, s in asks)
        )
    ...
    def reset(self) -> None:
        ...
        n_positions = sum(1 for p in self.ctx._positions.values() if p.qty_yes > 0 or p.qty_no > 0)
    ...
    async def _timer_loop(self) -> None:
        ...
        while True:
            ...
            positions = self.ctx.get_all_positions()
            open_positions = {
                cid: p for cid, p in positions.items() if p.qty_yes > 0 or p.qty_no > 0
            }
        ...
        while True:
            ...
            for provider in self.providers:
                ...
                if self._pool_refresh_cb is not None:
                    try:
                        await self._pool_refresh_cb(provider.name, provider.get_features())
                    except Exception:
                        logger.exception("pool_refresh_cb.error", provider=provider.name)
