...
class ResearchBriefRenderer:
    ...
    def render_escalation(
        ...
        # Header
        completed = sum(
            1 for s in tree.stages.values() if s.analysis is not None
        )
        ...
        trigger_text = (
            ...
            f"EVIDENCE: {', '.join(trigger.evidence)}"
        )
    ...
    def render_notification(self, result: BrainstormResult) -> None:
        ...
        for trigger in result.escalation_triggers:
            if trigger.severity == "notify":
                self.console.print(
                    f"[yellow]NOTICE:[/yellow] {trigger.description} "
...
class HumanInteraction:
    ...
    async def prompt(
        ...
        prompt_text = (
            f"Your choice ({'/'.join(valid_letters)}/custom/stop): "
        )
