...
class TagHRCopyStrategy:
    ...
    async def on_trade(
        ...
        if self._signal_variant == "buy_only":
            ...
            # Directional: BUY YES or SELL NO → bullish signal
            if trade.side == "BUY":
                outcome = "YES"
            elif trade.side == "SELL":
                # SELL NO = bullish (split mechanic)
                # SELL YES = bearish (skip for buy_only-directional)
                # For now, skip SELL for simplicity
                return None
            else:
                return None
