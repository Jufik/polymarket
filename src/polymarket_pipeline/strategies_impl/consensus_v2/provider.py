...
At startup (compute), queries the composite scorecard (excess_hr + consistency_sharpe
+ avg_edge_usd + bucket_excess_hr) and builds the top-K pool per tag.

Exposes:
  - pool: frozenset of trader addresses
  - tag_markets: frozenset of condition_ids for this tag
  - gambling_markets: frozenset of condition_ids to exclude
...
# Gambling slug patterns (shared with research build_pools.py)
_GAMBLING_PATTERNS = [
...
class ConsensusV2Provider:
    ...
    ----------
    tag:
        Tag label (e.g. "Sports", "Politics", "Crypto").
    k:
        Top-K traders to include in pool.
    lookback_months:
        Training window size in months.
    min_markets:
        Minimum resolved markets per trader.
    min_conviction:
    ...
    def _compute_sync(self) -> None:
        ...
        """)
        self._gambling_markets = frozenset(
            str(r[0]) for r in gambling_rows.result_rows
        )

        # 2. Tag markets (all, not just test period — live needs all active markets)
        tag_rows = ch.query(f"""
        ...
        # 3. Token map for direction resolution
        token_rows = ch.query("""
    ...
    def _build_composite_pool(
        self, ch: Any, train_start: str, train_end: str,
    ) -> set[str]:
        """Build composite-ranked pool using CH SQL."""
        tag = self._tag
        k = self._k
        ...
        # Consistency Sharpe (monthly HR, >=6 months with >=5 positions)
        ch.command(f"""
            ...
            FROM (
                SELECT
                    ...
                    avg(monthly.month_hr) / (if(stddevSamp(monthly.month_hr) > 0, stddevSamp(monthly.month_hr), 0.05) + 0.05) AS consistency_sharpe
                FROM (
                    SELECT
                        p.trader AS trader,
        ...
        # Composite score with percentile ranking
        rows = ch.query(f"""
            WITH base AS (
                SELECT
                    t.trader,
