...
class IntrospectServer:
    def __init__(self, runner: LiveRunner, port: int = 8002) -> None:
        self._runner = runner
        self._port = port
    ...
    async def start(self) -> None:
        ...
        try:
            self._runner_site = web.TCPSite(
                self._runner_obj, "0.0.0.0", self._port, reuse_address=True,
            )
            ...
            # Non-fatal — strategy continues without introspection
    ...
    def _get_strategy_internals(self) -> list[dict[str, Any]]:
        """Extract internal state from each strategy (duck-typed)."""
        results = []
        for strategy, config in self._runner.strategies:
            ...
            # ConsensusV2Strategy exposes _market_state and _signaled
    ...
    def _get_provider_summary(self) -> list[dict[str, Any]]:
        """Summarize each provider's state."""
        results = []
    ...
    async def _handle_pool(self, _request: web.Request) -> web.Response:
        """List all provider pools (summary only)."""
        results = []
        for provider in self._runner.providers:
            info: dict[str, Any] = {"name": provider.name}
            if hasattr(provider, "_tag"):
            ...
            if hasattr(provider, "_pool"):
                ...
                # Return truncated addresses for overview
                info["traders"] = sorted(
                    f"{addr[:8]}...{addr[-6:]}" for addr in pool
                )
            results.append(info)
        return web.json_response(results)

    async def _handle_pool_detail(self, request: web.Request) -> web.Response:
        """Full pool detail for a specific provider (full addresses)."""
    ...
    async def _handle_candidates(self, request: web.Request) -> web.Response:
        """Markets being tracked by strategies — trader counts, consensus, distance to threshold."""
        # Optional filter: ?strategy=politics_no_v3
        ...
        for strategy, config in self._runner.strategies:
            ...
            for cid, traders in strategy._market_state.items():
                # Build trader direction summary
                ...
                # Market question from context (if available)
                ...
                candidates.append({
                    ...
                    "would_pass_filter": (
                        direction_filter is None or consensus_dir == direction_filter
                    ),
                    ...
                    "traders": {
                        f"{t[:8]}...{t[-6:]}": d
                        for t, d in trader_dirs.items()
                    },
                })

        # Enrich missing questions from ClickHouse
        missing_cids = list({
            c["condition_id"] for c in candidates if c["question"] is None
        })
        if missing_cids:
        ...
        # Sort by distance to threshold (closest first), then by n_traders desc
    ...
    async def _handle_signaled(self, _request: web.Request) -> web.Response:
        """Markets that have already fired."""
        results = []
