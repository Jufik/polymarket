...
async def main() -> None:
    ...
    lockup = await query_ch(
        ...
        WITH
            ...
            ft AS (
                SELECT condition_id, toUnixTimestamp64Milli(min(timestamp)) / 1000.0 AS first_ts
                FROM (SELECT * FROM polymarket.trades_raw FINAL) t
                WHERE condition_id IN (SELECT condition_id FROM wm)
                GROUP BY condition_id
            )
        SELECT w.condition_id AS condition_id,
               (dateDiff('second', toDateTime64('1970-01-01', 3), e.end_date) - ft.first_ts) / 86400.0 AS lockup_days
        FROM wm w
