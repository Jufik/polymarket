...
def get_market_prices_at_signals(
    ...
    """Get the next available YES token price AFTER each signal trigger time (t+dt).

    Uses a FORWARD asof join: for each (condition_id, trigger_time), find the
    first price record at or after trigger_time + execution_delay_s. This
    simulates the realistic entry price when copying a trader's position.
    ...
    -------
    pl.DataFrame
        Signal table with trigger_entry_price replaced by forward market price.
    ...
    signals = signal_table.with_columns(
        (pl.col("trigger_time").dt.epoch("s").cast(pl.Float64) + execution_delay_s)
        .alias("trigger_ts")
    )
    ...
    # Forward asof join: for each signal row, find the first price >= trigger_ts
    ...
    if max_price_delay_s > 0:
        joined = joined.with_columns(
            pl.when(
                ...
                & ((pl.col("timestamp") - pl.col("trigger_ts")) <= max_price_delay_s)
            )
