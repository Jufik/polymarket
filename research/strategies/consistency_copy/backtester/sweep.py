...
def run_sweep(
    ...
    for min_t, agree, direction, (price_lo, price_hi) in combos:
        ...
        # Compute base rate for this direction (before sizing loop so Kelly can use it)
        br: float | None = None
        if base_rates is not None:
            if direction == "YES-only":
                ...
                n_yes_bets = bets.filter(pl.col("signal_direction") == "YES").height
                n_no_bets = bets.filter(pl.col("signal_direction") == "NO").height
        ...
        for sizing in config.sizing_strategies:
            # Pass base_rate as rolling_hr to avoid oracle look-ahead.
            # Without this, Kelly/edge_weighted use the actual full-window
            # hit rate (bets["won"].mean()), which is future information.
            sized = apply_sizing(
                bets,
            ...
            # Compute baseline daily PnL for base-rate adjustment
            ...
            row = {
                ...
                "sizing": sizing,
                "n_traders_in_signal": bets["trader"].n_unique(),
                **metrics,
            }
            results.append(row)
