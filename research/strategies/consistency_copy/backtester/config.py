...
class BacktestConfig:
    ...
    mvf_bands: list[str]
    max_median_entry_price: list[float]

    # [pricing]
    execution_delays: list[float]
    max_price_delay_s: float
    signal_price: bool  # include a sweep using trader's own entry price (no forward pricing)
...
def load_config(path: Path) -> BacktestConfig:
    ...
    return BacktestConfig(
        ...
        max_median_entry_price=[float(v) for v in pool["max_median_entry_price"]],
        # [pricing]
        execution_delays=[float(d) for d in pricing["execution_delays"]],
        max_price_delay_s=float(pricing["max_price_delay_s"]),
        signal_price=bool(pricing.get("signal_price", False)),
        # [sweep]
